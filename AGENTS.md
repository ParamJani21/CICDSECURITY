# CICDSECURITY - Agent Documentation
...
When PRs are opened or updated in GitHub, CICDSECURITY automatically:
...
### Check Database
```bash
sqlite3 cicdsecurity.db "SELECT * FROM scan_history ORDER BY created_at DESC LIMIT 5;"
```
...
- [ ] `.env` configured with App ID, private key, webhook secret
- [ ] ngrok token configured for public webhook access (optional)
- [ ] Security tools installed in WSL
- [ ] GitHub webhook configured in App settings
- [ ] Webhook URL set to: `https://your-ngrok-url.ngrok.io/github/webhook`
